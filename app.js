...
async function getHomeNews() {
    ...
    var topStoryObj = await processData(topStoryURL)
    console.log(topStoryObj.results[0]);
    let div = document.createElement('div')
    let a = document.createElement('a');

    let img = document.createElement('img')
    let imgURL = topStoryObj.results[0].multimedia[0].url;
    img.src = imgURL;
    img.width = 600;
    img.height = 300;
    div.innerHTML = `<div style="margin: left; text-allign:center; padding-left: 20%">
                    <h2>Today's Top Story:</h2>
                    <a href=${topStoryObj.results[0].url} target="_blank">
                        <img src=${img.src}
                        width=${img.width} height=${img.height} style="display: blocked">
                    </a>
                    <div style="margin-top:1%;  width: 80%; text-allign:"><h4>${topStoryObj.results[0].title}</h4></div>
                </div>`
    homeNewsDiv.appendChild(div);
}
